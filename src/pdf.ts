...
export async function exportSvgAsRaster(
  ...
  const raster = await rasterizeSvg(svgEl, format);
  download(raster.blob, filename);
}

export async function exportSvgAsPptx(svgEl: SVGSVGElement, filename = 'figure.pptx'): Promise<void> {
  const raster = await rasterizeSvg(svgEl, 'png');
  const pptx = buildPptxPackage(
    new Uint8Array(await raster.blob.arrayBuffer()),
    raster.width,
    raster.height,
  );
  download(new Blob([bytesToArrayBuffer(pptx)], {
    type: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
  }), filename);
}

export function exportSvgAsEditablePptx(svgEl: SVGSVGElement, filename = 'figure-editable.pptx'): void {
  const pptx = buildEditablePptxPackage(svgEl);
  download(new Blob([bytesToArrayBuffer(pptx)], {
    type: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
  }), filename);
}
...
export function buildEditablePptxPackage(svgEl: SVGSVGElement): Uint8Array {
  const viewBox = getSvgContentBox(svgEl);
...
function editablePptxShapes(svgEl: SVGSVGElement, viewBox: SvgViewBox, fit: PdfFitRect): string {
  ...
  const out: string[] = [];
  const elements = Array.from(svgEl.querySelectorAll('rect,circle,polygon,path,text,line,polyline'));
  for (const el of elements) {
    const tag = el.tagName.toLowerCase();
    if (tag === 'rect') out.push(...pptxRect(el as SVGRectElement, ctx));
    else if (tag === 'circle') out.push(...pptxCircle(el as SVGCircleElement, ctx));
    else if (tag === 'polygon') out.push(...pptxPolygon(el as SVGPolygonElement, ctx));
    else if (tag === 'path') out.push(...pptxPath(el as SVGPathElement, ctx));
    else if (tag === 'line') out.push(...pptxLine(el as SVGLineElement, ctx));
    else if (tag === 'polyline') out.push(...pptxPolyline(el as SVGPolylineElement, ctx));
    else if (tag === 'text') out.push(...pptxText(el as SVGTextElement, ctx));
  }
  return out.filter(Boolean).join('\n');
}

function pptxRect(el: SVGRectElement, ctx: PptxShapeContext): string[] {
...
function pptxCircle(el: SVGCircleElement, ctx: PptxShapeContext): string[] {
...
function pptxPolygon(el: SVGPolygonElement, ctx: PptxShapeContext): string[] {
...
function pptxPath(el: SVGPathElement, ctx: PptxShapeContext): string[] {
...
function pptxLine(el: SVGLineElement, ctx: PptxShapeContext): string[] {
...
function pptxPolyline(el: SVGPolylineElement, ctx: PptxShapeContext): string[] {
  const points = parsePoints(el.getAttribute('points'));
  if (points.length < 2) return [];
  const line = lineStyleFromElement(el, ctx.tx);
  const out: string[] = [];
...
function pptxText(el: SVGTextElement, ctx: PptxShapeContext): string[] {
...
function mapBox(tx: PptxTransform, x: number, y: number, w: number, h: number): { x: number; y: number; w: number; h: number } {
  ...
  return {
    x: a.x,
    y: a.y,
    w: Math.max(1, Math.round(w * tx.scaleX)),
    h: Math.max(1, Math.round(h * tx.scaleY)),
  };
}
...
function colorAttr(el: Element, name: string, fallback: string): string {
...
function lineStyleFromElement(el: Element, tx: PptxTransform): PptxLineStyle | null {
...
function fillXml(color: string): string {
  return color === 'none'
    ? '<a:noFill/>'
    : `<a:solidFill><a:srgbClr val="${xmlAttr(color)}"/></a:solidFill>`;
}
...
<Application>PatentDSL</Application>
...
<dc:title>PatentDSL figure</dc:title>
<dc:creator>PatentDSL</dc:creator>
<cp:lastModifiedBy>PatentDSL</cp:lastModifiedBy>
...
function themeXml(): string {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="PatentDSL">
<a:themeElements>
<a:clrScheme name="PatentDSL"><a:dk1><a:srgbClr val="000000"/></a:dk1><a:lt1><a:srgbClr val="FFFFFF"/></a:lt1><a:dk2><a:srgbClr val="000000"/></a:dk2><a:lt2><a:srgbClr val="FFFFFF"/></a:lt2><a:accent1><a:srgbClr val="0B5FFF"/></a:accent1><a:accent2><a:srgbClr val="666666"/></a:accent2><a:accent3><a:srgbClr val="999999"/></a:accent3><a:accent4><a:srgbClr val="CCCCCC"/></a:accent4><a:accent5><a:srgbClr val="333333"/></a:accent5><a:accent6><a:srgbClr val="111111"/></a:accent6><a:hlink><a:srgbClr val="0B5FFF"/></a:hlink><a:folHlink><a:srgbClr val="0B5FFF"/></a:folHlink></a:clrScheme>
<a:fontScheme name="PatentDSL"><a:majorFont><a:latin typeface="Arial"/><a:ea typeface="Yu Gothic"/><a:cs typeface="Arial"/></a:majorFont><a:minorFont><a:latin typeface="Arial"/><a:ea typeface="Yu Gothic"/><a:cs typeface="Arial"/></a:minorFont></a:fontScheme>
<a:fmtScheme name="PatentDSL"><a:fillStyleLst><a:solidFill><a:schemeClr val="phClr"/></a:solidFill></a:fillStyleLst><a:lnStyleLst><a:ln w="9525" cap="flat" cmpd="sng" algn="ctr"><a:solidFill><a:schemeClr val="phClr"/></a:solidFill><a:prstDash val="solid"/></a:ln></a:lnStyleLst><a:effectStyleLst><a:effectStyle><a:effectLst/></a:effectStyle></a:effectStyleLst><a:bgFillStyleLst><a:solidFill><a:schemeClr val="phClr"/></a:solidFill></a:bgFillStyleLst></a:fmtScheme>
</a:themeElements>
<a:objectDefaults/>
<a:extraClrSchemeLst/>
</a:theme>`;
